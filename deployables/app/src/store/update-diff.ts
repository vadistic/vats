...
import { applyChange, diff, Diff, PreFilterFunction } from 'deep-diff'
...
export const updateDiff = <Value>(
  prev: Value,
  next: Value,
  { map, prefilter }: UpdateDiffOptions,
) => {
  ...
  const updateDiffs = diff<Value, Value>(
    prev,
    next,
    (path, key) => (prefilter && prefilter(path, key)) || path.length >= 4,
  )
