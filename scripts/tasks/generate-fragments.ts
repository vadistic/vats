...
const nonNull = <T>(input: T): input is NonNullable<T> => input !== null
...
class GenerateFragments {
  ...
  private makeFragments(schemaContents: string, generator: string, deepNesting: boolean) {
    ...
    // naive regex way to find imported fragments names
    const spreadRegex = /\.\.\.([A-z-_0-9]+)\r?\n/
    const getFragmentImports = (fields: string[]) => {
      const result: string[] = []
      let match
      fields.forEach(field => {
        match = spreadRegex.exec(field)
        if (match) {
          result.push(match[1])
        }
      })
    ...
    const standardFragments: Fragment[] = typeNames
      .map(typeName => {
        ...
        return {
          name,
          imports: getFragmentImports(fields),
          fragment,
        }
      })
      .filter(nonNull)

    const noRelationsFragments: Fragment[] = typeNames
      .map(typeName => {
        const type = ast.getType(typeName)
    ...
    const deepFragments: Fragment[] = !deepNesting
      ? []
      : typeNames
          .map(typeName => {
            const type = ast.getType(typeName)
    ...
    if (generator === 'js') {
      const printImports = (imports: string[]) =>
        imports.length > 0
          ? '\n\n' + imports.map(interpolation => '${' + interpolation + 'Fragment}').join('\n')
          : ''
